#!/usr/bin/env python3
"""
Android APK SAST Tool - SAST_AndroTX_55 v1.2
...
v1.2 Improvements:
- Enhanced file filtering to exclude binary files (fonts, images, audio, video, etc.)
- Improved path traversal patterns with context awareness
- Binary file detection to prevent regex matching on binary data
- Context-aware pattern matching to distinguish code from other text
- Post-processing false positive filtering
- Reduced noise from font files and other binary assets
"""
...
class APKParser:
    ...
    def read_file_text_best_effort(self, inner_path: str) -> str:
        try:
            ...
            # Check if file appears to be binary by looking for null bytes or high ratio of non-printable chars
            if len(data) > 0:
                null_count = data.count(b'\x00')
                non_printable_count = sum(1 for b in data if b < 32 and b not in [9, 10, 13])  # Exclude tab, LF, CR
                
                # If more than 10% null bytes or 30% non-printable chars, likely binary
                if null_count > len(data) * 0.1 or non_printable_count > len(data) * 0.3:
                    return ""
    ...
    def iter_candidate_text_files(self) -> List[str]:
        ...
        binary_extensions = {
            ...
            '.dex', '.class', '.jar',  # Compiled code
            '.apk', '.aab',  # Android packages
            '.db', '.sqlite', '.sqlite3',  # Databases
            '.bin', '.dat', '.cache', '.tmp'  # Binary data
        }
        ...
        for n in names:
            ...
            # Include text files by extension
            if lower.endswith(('.xml', '.txt', '.json', '.yaml', '.yml', '.properties', '.cfg', '.ini', '.pem', '.crt', '.conf', '.js', '.java', '.kt')):
                candidates.append(n)
            # Include specific directories that might contain text files
            elif lower.startswith(('assets/', 'res/raw/', 'res/xml/', 'META-INF/', 'lib/')):
                # Additional check: skip if it looks like a binary file
                if not any(lower.endswith(ext) for ext in binary_extensions):
                    candidates.append(n)
        return candidates
...
class MobileVulnerabilityDetector:
    ...
    def _load_rules(self) -> Dict[str, Dict[str, Any]]:
        return {
            ...
            'M4-002': {
                ...
                'severity': Severity.MEDIUM,
                'patterns': [
                    r'File\s*\(\s*[^)]*\.\./[^)]*\)',  # File constructor with ../
                    r'openFileOutput\s*\(\s*[^)]*\.\./[^)]*\)',  # openFileOutput with ../
                    r'getExternalFilesDir\s*\(\s*[^)]*\.\./[^)]*\)',  # getExternalFilesDir with ../
                    r'new\s+File\s*\(\s*[^)]*\.\./[^)]*\)',  # new File with ../
                    r'\.\./.*\.(txt|xml|json|properties|conf)',  # .. followed by common file extensions
                    r'\.\.\\[^\\]*\.(txt|xml|json|properties|conf)',  # Windows style with file extensions
                ],
    ...
    def _is_likely_code_file(self, file_path: str, content: str) -> bool:
        """Determine if a file is likely to contain code based on path and content."""
        lower_path = file_path.lower()
        
        # Check file extension
        code_extensions = {'.java', '.kt', '.js', '.xml', '.json', '.properties', '.conf'}
        ...
        code_indicators = [
            ...
            r'class\s+\w+',  # General class
            r'function\s+\w+',  # JavaScript function
            r'<activity\s+',  # Android activity
            r'<service\s+',  # Android service
    ...
    def _is_likely_false_positive(self, rule_id: str, line: str, file_path: str, is_likely_code: bool) -> bool:
        """Check if a finding is likely a false positive based on context."""
        
        # Skip findings in non-code files for certain rules
        if not is_likely_code and rule_id in ['M4-002', 'M1-001', 'M5-001']:
            return True
            
        # Skip if line is mostly non-printable characters (binary data)
        printable_chars = sum(1 for c in line if c.isprintable() or c.isspace())
        if len(line) > 0 and printable_chars / len(line) < 0.7:
            return True
            
        # Skip if line is too short or looks like binary data
        if len(line.strip()) < 3:
            return True
            
        # Skip if line contains mostly special characters (likely binary)
        special_chars = sum(1 for c in line if not c.isalnum() and not c.isspace())
        if len(line) > 0 and special_chars / len(line) > 0.8:
            return True
            
        # Skip path traversal findings that don't look like actual file operations
        if rule_id == 'M4-002':
            # Skip if it's just ".." without any file operation context
    ...
    def detect_in_files(self, parser: APKParser) -> List[Finding]:
        ...
        for inner_path in candidates:
            ...
            # Skip files that are too short or look like binary data
            if len(text) < 10:
                continue
                
            # Check if content looks like code vs other text
            is_likely_code = self._is_likely_code_file(inner_path, text)
                ...
                for pattern in rule.get('patterns', []) or []:
                    ...
                    for idx, line in enumerate(lines, 1):
                        if regex.search(line):
                            ...
                            if self._is_likely_false_positive(rule_id, line, inner_path, is_likely_code):
                                continue
    ...
    def _filter_false_positives(self, findings: List[Finding]) -> List[Finding]:
        """Post-process findings to remove obvious false positives."""
        filtered = []
        
        for finding in findings:
            # Skip if file path indicates it's a binary file
            if any(finding.file_path.lower().endswith(ext) for ext in ['.ttf', '.otf', '.png', '.jpg', '.jpeg', '.gif', '.mp3', '.mp4', '.so', '.dex']):
                continue
                
            # Skip if code snippet looks like binary data
            if len(finding.code_snippet) > 0:
                printable_ratio = sum(1 for c in finding.code_snippet if c.isprintable() or c.isspace()) / len(finding.code_snippet)
                if printable_ratio < 0.7:
                    continue
                    
            # Skip if code snippet is too short or meaningless
            if len(finding.code_snippet.strip()) < 3:
                continue
                
            # Skip path traversal findings in non-code contexts
            if finding.rule_id == 'M4-002':
                # Skip if it's just isolated ".." without file operation context
                if re.match(r'^\s*\.\.\s*$', finding.code_snippet.strip()):
                    continue
                # Skip if it's in what looks like binary data
                if finding.code_snippet.count('.') > len(finding.code_snippet) * 0.3:
                    continue
...
def main() -> None:
    parser = argparse.ArgumentParser(description='Android APK SAST Tool (SAST_AndroTX_55 v1.2)')
